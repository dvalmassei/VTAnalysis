...
def read_signals(data_folder, runs, channels):
    ...
    dfs = []
    dfs.append(pd.read_csv(data_folder + f'/{runs[0]}/TR_0_0.txt', header=None,dtype=float,skiprows=1))
    for ch in channels:
        dfs.append(pd.read_csv(data_folder + f'/{runs[0]}/wave_{ch}.txt', header=None,dtype =float,skiprows=1))
    
    if len(runs) > 1:
        print('more than one run')
...
def integrate_signals(channels, events, signalThreshold, signals, offsets, atten):
    ...
    for i in range(len(events)):
        ...
        offsets = [np.mean(signals[ch+1][start_index-60:start_index]) for ch in range(len(channels))]
        for ch in range(len(channels)):
            if (max(offsets[ch] - signals[ch+1][start_index:start_index + 800])> signalThreshold):
                charge[ch][i] = sum(offsets[ch] - signals[ch+1][start_index:start_index + 800]) / (5.12*1E9) * 1E12 / atten
                #charge[-1][i] = sum(offsets[-1] - signals[-1][start_index:start_index + 1000]) / (5*1E9) * 1E12 / atten
...
def histogram_charges(charge, channels, histEndpoint, nBins):
    ...
    bins = []
    for i in range(len(channels)):
        hist0, bins0 = np.histogram(charge[i][charge[i] != 0], bins=nBins, range=(-10 , histEndpoint))
        hists.append(hist0)
...
def main(data_folder, runs, data_channels, noise_channel, atten, triggerThreshold, signalThreshold, nBins, histEndpoint):
    ...
    print(f'Max Charge: {np.max(charge)}')
    ...
    with open('chargeHistogram.csv', 'w', encoding="ISO-8859-1", newline='') as myfile:
...
if __name__ =='__main__':
    ...
    atten = 1.0 #[Amplification]*[Attenuation Factor] #Assume 1.0 Attenuation Indefinetly 
    triggerThreshold = 100
    signalThreshold = 0.000 #0.001 #in Amps
    nBins = 128
    histEndpoint = 500
